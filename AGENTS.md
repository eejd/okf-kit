...
`okf-kit` implements the **Open Knowledge Format (OKF v0.1)** as **agent-native tooling**, not a hosted web app. An OKF bundle is a directory of Markdown files (YAML frontmatter + body); each file is one *concept*; the file path is its id; relative Markdown links form a knowledge graph. See `_docs/requirements.md` for the full spec — the **data model and conformance rules (§2–4) are authoritative and unchanged**.
...
**v0.1 scope = build + use a KB:** scaffold (`okf init`), author (`okf new` + the `okf-author` skill), validate (SPEC §9), search, progressive-context read, regenerate `index.md` — shipped as a Python core + `okf` CLI + a 3-tool `okf-mcp` server + the `okf-author` skill. The full pack, single-file viewer, producer, governance, and multi-level federation are deferred — see `_docs/2026-06-16-okf-plugin-v0.1-design.md` §2/§13.
...
- **Tools well documented** — every tool is documented in three synced places: MCP tool `description`, CLI `--help`, and `docs/tools.md`. Design §11.
...
│   ├── mcp.py            # `okf-mcp` server (search / read_concept / validate + okf:// resources)
...
docs/                     # tools.md, progressive-context.md, okf-uri-scheme.md, authoring.md (v0.1)
...
- **Authoring is CLI + skill, not MCP.** Scaffold with `okf init`, create concepts from type templates with `okf new <bundle> <type> <path>`, then `okf validate` + `okf index regen`. The `okf-author` skill runs the author → link → validate → index loop; the agent writes `.md` files directly (OKF is just markdown). MCP write tools are deferred.
- **Links:** extract **relative** Markdown links only; resolve against the source doc's directory; normalize to concept ids (strip `.md`); drop edges to non-existent concepts silently (SPEC §5.3).
...
- **Tool docs stay in sync.** The canonical description for each tool lives in `docs/tools.md` and is copied verbatim into the MCP `description` and CLI `--help`; a test asserts they match.
...
- Tool reference (once built): `docs/tools.md`, `docs/progressive-context.md`, `docs/okf-uri-scheme.md`
